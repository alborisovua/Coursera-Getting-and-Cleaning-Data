...
2. Unzip the file.
3. Move all files and folders from "UCI HAR Dataset" to the SAME DIRECTORY as the R script:
	* `features.txt`
	* `subject_train.txt`
	* `subject_test.txt`
	* `X_train.txt`
...
**Once those steps are complete, you can run the R script ([run_analysis.R](run_analysis.R)).** Note that it requires data.table and dplyr packages, which can be downloaded from CRAN.
